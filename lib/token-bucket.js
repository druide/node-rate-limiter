...
class TokenBucket {
  ...
  constructor (bucketSize: number, tokensPerInterval: number, interval: string|number) {
  ...
  drip () {
    ...
    let deltaMS: number = Math.max(now - this.lastDrip, 0)
    this.lastDrip = now

    let dripAmount: number = deltaMS * (this.tokensPerInterval / this.interval)
